...
\begin{itemize}[leftmargin=1.5em]
    \item When $\rho = 0$, the map is the identity: $(\omega^1, (1-\omega)^1) = (\omega, 1-\omega)$. We move in a straigh line. 
...
% FIGURE PLACEHOLDER: Unit box with diagonal (simplex) and curves for rho = -1, 0, 0.5, 0.9
\begin{center}
\textit{[Figure: Curves $(\omega^{1-\rho}, (1-\omega)^{1-\rho})$ in the unit box for various $\rho$]}
\end{center}
...
Using this transformation, we obtain a relationship between the cannonical and the classical means.
...
\begin{equation}
\label{eq:Omega-def}
\Omega(\bom, \rho) = \left( \sum_{j=1}^{n} \omega_j^{1-\rho} \right)^{1/\rho} = \M(\mathbf{1}; \bom, \rho).
\end{equation}
\end{proposition}
